...
bool ComparePropertyLess(const UEProperty& lhs, const UEProperty& rhs)
{
	if (lhs.GetOffset() == rhs.GetOffset()
		&& lhs.field->ClassPrivate->Id == FFieldClassID::Bool
		&& rhs.field->ClassPrivate->Id == FFieldClassID::Bool)
	{
...
void Package::GeneratePrerequisites(const UEObject& obj, std::unordered_map<UEObject, bool>& processedObjects)
{
	...
	auto objAsObj = ((UObject*)obj.GetAddress());

	if (objAsObj->ObjectFlags & 0x00000010)
		return;
	...
	if (processedObjects[obj] == false)
	{
		...
		GenerateMemberPrerequisites(structObj.GetChildProperties().Cast<UEProperty>(), processedObjects);
...
void Package::GenerateMemberPrerequisites(const UEProperty& first, std::unordered_map<UEObject, bool>& processedObjects)
{
	...
	for (auto prop = first; prop.IsValid(); prop = prop.GetNext().Cast<UEProperty>())
	{
		...
		{
			if (prop.field->ClassPrivate->Id == FFieldClassID::Byte)
			{
			...
			else if (prop.field->ClassPrivate->Id == FFieldClassID::Enum)
			{
		...
		else if (info.Type == UEProperty::PropertyType::Container)
		{
			...
			if (prop.field->ClassPrivate->Id == FFieldClassID::Array)
			{
				innerProperties.push_back(prop.Cast<UEArrayProperty>().GetInner());
			}
			else if (prop.field->ClassPrivate->Id == FFieldClassID::Map)
			{
...
void Package::GenerateScriptStruct(const UEScriptStruct& scriptStructObj)
{
	...
	for (auto prop = scriptStructObj.GetChildProperties().Cast<UEProperty>(); prop.IsValid(); prop = prop.GetNext().Cast<UEProperty>())
	{
...
{
	auto objAsObj = ((UObject*)enumObj.GetAddress());
...
{
	auto objAsObj = ((UObject*)constObj.GetAddress());
...
void Package::GenerateClass(const UEClass& classObj)
{
	...
	{
		std::vector<UEProperty> properties;
		for (auto prop = classObj.GetChildProperties().Cast<UEProperty>(); prop.IsValid(); prop = prop.GetNext().Cast<UEProperty>())
		{
			if (prop.GetElementSize() > 0
				&& (!super.IsValid()
					|| (super != classObj
...
void Package::GenerateMembers(const UEStruct& structObj, size_t offset, const std::vector<UEProperty>& properties, std::vector<Member>& members) const
{
	...
	for (auto&& prop : properties)
	{
		...
		if (info.Type != UEProperty::PropertyType::Unknown)
		{
			...
			if (prop.field->ClassPrivate->Id == FFieldClassID::Bool && prop.Cast<UEBoolProperty>().IsBitfield())
			{
...
void Package::GenerateMethods(const UEClass& classObj, std::vector<Method>& methods) const
{
	...
	for (auto func = classObj.GetChildren().Cast<UEFunction>(); func.IsValid(); func = func.GetNext().Cast<UEFunction>())
	{
		if (!func.IsA<UEFunction>())
			continue;
		...
		for (auto param = function.GetChildProperties().Cast<UEProperty>(); param.IsValid(); param = param.GetNext().Cast<UEProperty>())
		{
			...
			if (info.Type != UEProperty::PropertyType::Unknown)
			{
				...
				if (param.field->ClassPrivate->Id == FFieldClassID::Bool)
				{
